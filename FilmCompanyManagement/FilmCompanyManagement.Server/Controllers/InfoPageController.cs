...
namespace FilmCompanyManagement.Controllers
{
    ...
    public class InfoPageController : ControllerBase
    {
        ...
        {//worker:报销申请
            return Ok(await _context.FundingApplications.Include(fa => fa.Employee).Include(fa => fa.Bill).Where(fa => fa.Employee.UserName == requestBody.id).ToListAsync());
        }

        [HttpPost]
        public async Task<ActionResult> GetEquipments(thisRequest2 requestBody)
        {//worker:设备购买
            var unfinishedProjects = await _context.PhotoEquipments.Include(p => p.Employee).Include(p => p.Bill).Where(p => p.Employee.UserName == requestBody.id).ToListAsync();
        ...
        {//worker:设备维修
            var unfinishedProjects = await _context.EquipmentRepairs.Include(p => p.Employee).Include(p => p.PhotoEquipment).Include(p => p.Bill).Where(p => p.Employee.UserName == requestBody.id).ToListAsync();
            return Ok(unfinishedProjects);
        }
        ...
        {//worker:培训通知
            var user = await _context.Employees.Where(d => d.UserName == requestBody.id).SingleAsync();
        ...
        {//boss:报销申请
            var workers = await _context.Employees.Include(e => e.Department).Include(e => e.Department.Leader).Where(e => e.Department.Leader.UserName == requestBody.id).ToListAsync();
        ...
        {
            return Ok(await _context.Attendances.Include(a => a.Employee).Where(a => a.Employee.UserName == requestBody.id ).FirstOrDefaultAsync() == null);
        }

        [HttpPost]
        public async Task<ActionResult> InsertAttendenceRecord(stateRequest requestBody)
        {
            var user = await _context.Employees.Where(e => e.UserName == requestBody.id).SingleAsync();
            if (await _context.Attendances.Include(a => a.Employee).FirstOrDefaultAsync(a => a.Employee.UserName == requestBody.id && a.Date == DateTime.Now) == null)
            {
        ...
        public async Task<ActionResult> GetKPIs(thisRequest2 requestBody)
        {
            var KPIs = await _context.KPI.Include(k => k.Project).Include(k => k.Project.Manager).
                Where(k => k.Project.Manager.UserName == requestBody.id).ToListAsync();
            if (KPIs.Count == 0)
                return Ok();
            ...
            var latestKPI = await _context.KPI.Include(k => k.Project).Include(k => k.Project.Manager).Include(k => k.Project.Bill).Include(k => k.Project.Customer).Include(k => k.Project.Employees).
                Where(k => k.Project.Manager.UserName == requestBody.id && (long)k.Date.Subtract(new DateTime(1970, 1, 1)).TotalDays == latestDate).SingleAsync();
