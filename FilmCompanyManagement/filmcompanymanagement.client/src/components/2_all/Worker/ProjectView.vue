...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            jump(event) {
                const id = this.$route.query.id || '3'; // 默认 id 为 '3'
                switch (event.target.id) {
                    ...
                    case 'menu_1':
                        ...
                        break;
                    case 'menu_2':
                        this.$router.push({ path: '/projects', query: { id } });
                        break;
                    case 'menu_3':
            ...
            getProjects() {
                const userID = this.$route.query.id;
                axios.get('/api/worker/projects')
                    .then(response => {
                        this.projects = response.data.projects.filter(project => project.workerID === userID);
                    })
                    .catch(error => {
